...
excel_utils.py stores its data as tabs (worksheets) inside ONE Google
Sheet instead of local .xlsx files. This fixes the Render free-tier
issue where the local filesystem is wiped on every restart/redeploy/
spin-down — a Google Sheet lives on Google's servers, so it survives
all of that, for free, forever, and needs no database at all.

Required environment variables (set these in Render → your service →
Environment, and in a local .env for development):
  ...
  GOOGLE_SERVICE_ACCOUNT_JSON
      The full contents of the service-account JSON key file.
...
def _load_credentials():
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "pasted the *entire* key file contents (see GOOGLE_SHEETS_SETUP.md)."
        ) from e
...
def get_or_create_worksheet(name, headers):
    ...
    If it exists but is missing some headers, the missing ones are
    appended so older sheets keep working without manual editing."""
    ss = get_spreadsheet()
    with _lock:
        try:
            ...
            ws = ss.add_worksheet(
                title=name, rows=500, cols=max(len(headers), 10)
            )
